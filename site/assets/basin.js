...
(function () {
  ...
  // размахи с осциллограммы опыта, мм (19 значений учебного примера)
  ...
  // источник данных обработки: натурный опыт или снятая виртуальная осциллограмма
  ...
  function lab1() {
    ...
    // «испр»-значения: у натурного опыта — снятые с кривых, у виртуального — МНК-сглаживание
    ...
    if (link && isFinite(nu2dim)) {
      ...
      $('vx1-toroll-note').innerHTML = `В расчёт качки уйдут ${virtual ? 'снятые в виртуальном опыте' : 'натурные'} значения:
  ...
  // монотонная кубическая интерполяция (Fritsch–Carlson) R(v) через точки опыта
