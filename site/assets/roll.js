...
(function () {
  ...
  function recalc() {
    ...
    if (mode === 'work') {
      o.push(`<div class="small" style="margin-top:8px">Упрощённый режим: дальше используются
      эффективные константы числового примера ω_θ = <b>${fmt(K.wt, 4)}</b>, 2ν_θ = <b>${fmt(K.nu2, 4)}</b>,
      λ̄₄₄ = <b>${fmt(K.lamBar, 4)}</b>, R̄ = <b>${fmt(K.Rb, 4)}</b>, 2ν_θк = <b>${fmt(K.nu2k, 4)}</b>
      (промежуточные величины округляются иначе; расхождение ≤ 0,7 %).</div>`);
    }
